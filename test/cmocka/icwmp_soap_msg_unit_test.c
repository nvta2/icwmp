...
void unit_test_remove_all_session_events()
{
...
static void prepare_session_for_rpc_method_call()
{
	...
	xml_prepare_msg_out(cwmp_main->session);
}
...
static void prepare_spv_soap_request(char *parameter, char *value, char *parameter_key)
{
	mxml_node_t *params = NULL, *n = NULL;

	cwmp_main->session->tree_in = mxmlLoadString(NULL, CWMP_SETPARAMETERVALUES_REQ, MXML_OPAQUE_CALLBACK);
	xml_recreate_namespace(cwmp_main->session->tree_in);
	params = mxmlFindElement(cwmp_main->session->tree_in, cwmp_main->session->tree_in, "ParameterValueStruct", NULL, NULL, MXML_DESCEND);
	...
	n = mxmlFindElement(cwmp_main->session->tree_in, cwmp_main->session->tree_in, "ParameterKey", NULL, NULL, MXML_DESCEND);
...
static void soap_set_param_value_message_test(void **state)
{
	...
	*/
	...
	prepare_spv_soap_request("Device.ManagementServer.PeriodicInformEnable", "true", "set_value_test");
	prepare_session_for_rpc_method_call();

	int ret = cwmp_handle_rpc_cpe_set_parameter_values(rpc_cpe);
	assert_int_equal(ret, 0);
	...
	MXML_DELETE(cwmp_main->session->tree_in);
	MXML_DELETE(cwmp_main->session->tree_out);
	cwmp_main->session->tree_out = NULL;
	...
	prepare_spv_soap_request("Device.ManagementServeriodicInformEnable", "mngmt_enable", "set_value_test");
	prepare_session_for_rpc_method_call();

	ret = cwmp_handle_rpc_cpe_set_parameter_values(rpc_cpe);
	assert_int_equal(ret, 0);
	...
	MXML_DELETE(cwmp_main->session->tree_in);
	MXML_DELETE(cwmp_main->session->tree_out);
	cwmp_main->session->tree_in = NULL;
	cwmp_main->session->tree_out = NULL;
	 ...
	 */
	rpc_cpe->type = RPC_CPE_SET_PARAMETER_VALUES;
	prepare_spv_soap_request("Device.ManagementServer.AliasBasedAddressing", "true", "set_value_test");
	prepare_session_for_rpc_method_call();
	...
	prepare_spv_soap_request("Device.ManagementServer.PeriodicInformEnable", "false", INVALID_PARAM_KEY);
	prepare_session_for_rpc_method_call();
...
static void prepare_download_soap_request(char *url, char *file_type, char *username, char *password, char *file_size, char *delay_second)
{
	mxml_node_t *n = NULL, *download = NULL;
	cwmp_main->session->tree_in = mxmlLoadString(NULL, CWMP_DOWNLOAD_REQ, MXML_OPAQUE_CALLBACK);
	xml_recreate_namespace(cwmp_main->session->tree_in);
	download = mxmlFindElement(cwmp_main->session->tree_in, cwmp_main->session->tree_in, "cwmp:Download", NULL, NULL, MXML_DESCEND);
...
static void soap_download_message_test(void **state)          //TODO will be properly done with Download unit tests
{
	...
	* Valid Arguments
	*/
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "6 Stored Firmware Image", "iopsys", "iopsys", "0", "0");
	prepare_session_for_rpc_method_call();
	int ret = cwmp_handle_rpc_cpe_download(rpc_cpe);
	assert_int_equal(ret, 0);
	env = mxmlFindElement(cwmp_main->session->tree_out, cwmp_main->session->tree_out, "soap_env:Envelope", NULL, NULL, MXML_DESCEND);
	assert_non_null(env);
	n = mxmlFindElement(env, env, "soap_env:Header", NULL, NULL, MXML_DESCEND);
	...
	mxml_node_t *cwmp_fault = NULL;
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "7 New FileType", "iopsys", "iopsys", "0", "0");
	prepare_session_for_rpc_method_call();
	ret = cwmp_handle_rpc_cpe_download(rpc_cpe);
	assert_int_equal(ret, 0);
	 ...
	 * Wrong FileSize
	 */
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "6 Stored Firmware Image", "iopsys", "iopsys", "-15", "0");
	prepare_session_for_rpc_method_call();
	ret = cwmp_handle_rpc_cpe_download(rpc_cpe);
	assert_int_equal(ret, 0);
	...
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "6 Stored Firmware Image", "iopsys", "iopsys", "0", "jjjk");
	prepare_session_for_rpc_method_call();
	ret = cwmp_handle_rpc_cpe_download( rpc_cpe);
	assert_int_equal(ret, 0);
	 ...
	 * Invalid username
	 */
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "6 Stored Firmware Image", INVALID_USER, "iopsys", "0", "0");
	prepare_session_for_rpc_method_call();
	ret = cwmp_handle_rpc_cpe_download(rpc_cpe);
	assert_int_equal(ret, 0);
	 ...
	 * Invalid password
	 */
	prepare_download_soap_request("http://192.168.1.160/tr069/DG400PRIME-A-IOPSYS-6.2.0BETA1-210530_2348_nand_squashfs_update.pkgtb", "6 Stored Firmware Image", "iopsys", INVALID_PASS, "0", "0");
	prepare_session_for_rpc_method_call();
	ret = cwmp_handle_rpc_cpe_download(rpc_cpe);
	assert_int_equal(ret, 0);
